...
void CheckActuatorCommands() {
    ...
    const auto levelMove = CalculateMotion({}, neutral);
    Require(levelMove.positions == levelTargets, "Level-pose calibration changed");
    Require(levelMove.speeds == ActuatorValues{{20, 20, 20, 20, 20, 20}}, "Level-pose speeds changed");
    const auto stationary = CalculateMotion({}, levelTargets);
    Require(stationary.positions == levelTargets, "Settled platform must retain its targets");
    ...
    const ActuatorValues displaced{{0, 400, 200.75f, 201.25f, 100, 300}};
    const auto returnToNeutral = CalculateMotion({}, displaced);
    Require(returnToNeutral.positions == ActuatorValues{{20, 380, 201, 201, 120, 280}}, "Step limit must be relative to feedback");
