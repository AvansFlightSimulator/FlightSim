...
This Windows application connects Microsoft Flight Simulator or manual HMI input to either a PLC or a Unity client. It reads aircraft orientation and rudder data through the MSFS SimConnect SDK, translates that motion into final position targets and Point-to-Point velocity limits for six platform actuators, and exchanges newline-delimited JSON over TCP.
...
5. The final actuator target is recalculated every nominal 50 ms and sent directly with a 400 controller-unit/s Point-to-Point velocity limit. Controller feedback gates output but does not create intermediate targets. To request level, enter zero in all three fields and press Execute.
...
show feedback, the final command target, and its Point-to-Point velocity limit.
Every nominal 50 ms calculation republishes the applied final target directly;
feedback does not advance it through intermediate positions. Invalid entries reject
the entire new request and leave the previous applied request active.
...
4. With valid controller feedback available, each final actuator target is clamped to the existing 0..999 controller range and paired with the configured 400 controller-unit/s Point-to-Point velocity limit. `ControllerProtocol` encodes the command as PLC or Unity JSON, which `MotionController` publishes as the latest payload.
...
PLC / Unity -> TCPServer -> ControllerProtocol -> position feedback -> HMI and output gate
```
...
- `MotionCalculator.h/.cpp`: simulator-to-platform angle mapping, platform mounting coordinates, calibration constants, final target clamping, the Point-to-Point velocity limit, and the control interval. No SDK or networking dependencies.
...
- `tests/motion_calculator_tests.cpp`: existing angle mapping, calibrated targets, actuator order, final target commands, controller bounds, and Point-to-Point velocity checks.
...
- `tests/motion_controller_tests.cpp`: live-filter pipeline integration, manual angle and PLC actuator execution without MSFS, source isolation, validation, feedback gating, cadence, rounding, and target replacement during an unfinished move; requires nlohmann/json.
...
3. **Calculate the legs:** `CalculateMotion` takes the platform attitude. The geometry applies translation plus a rotated platform mounting point minus a base mounting point. The vector's length becomes a final target in the existing controller reference system. Geometry uses yaw about Z, negative platform roll about Y, and platform pitch about X; preserve this established mapping.
4. **Build the Point-to-Point command:** each final target is clamped to the existing 0..999 controller range and sent directly. Every actuator receives a 400 controller-unit/s velocity limit, preserving the effective maximum of the former ramp without deriving velocity from position error or the 50 ms update interval. A level pose currently rounds to 201 on all six actuators, although the reference constant is 200. These are existing calibration results, not new physical-unit claims.
...
6. **Receive controller feedback:** `TCPServer::receiveData` reads bytes. `FeedbackStream` assembles complete messages. `TryParseFeedback` validates all six values before the server replaces its synchronized position array and updates the HMI. The first valid sample enables output for that connection. Feedback is not used to build a position ramp.

The three threads have distinct jobs:

| Thread | Owns/does | Shared data access |
| --- | --- | --- |
| Main | HMI controls/message loop, SimConnect session or manual calculation ticks, attitude mapping and motion calculation | Reads the feedback-valid gate; publishes the latest payload; updates dashboard |
...
- Current position, final commanded target, and commanded Point-to-Point velocity limit for all six actuators.
...
- Motion constants such as attitude limits and the 400 controller-unit/s Point-to-Point velocity limit are grouped at the top of `MotionCalculator.cpp` for review and tuning.
...
The earlier actuator-position validation in September 2026 predates the direct
Point-to-Point target change: Debug/Release x64 builds and all five suites passed.
The motion-controller suite also passed in an isolated Unity build, including
rejection of unsupported direct-position mode. A loopback PLC run reached six
different requested positions under the former feedback-relative ramp and passed
blank/malformed/nonfinite/range rejection, draft isolation, all three
...
- In PLC actuator-position mode, verify A1-A6 ordering, immediate final targets, target replacement before the prior move completes, invalid-input rejection, and a new Execute requirement after switching modes.
